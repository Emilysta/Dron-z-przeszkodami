...
namespace PzG {
 ...
 std::list<InfoFileToDraw>  LaczeDoGNUPlota::_InfoPlikow;
 ...
 //
 bool LaczeDoGNUPlota::DodajNazwePliku( const char * NazwaPliku,DrawingType RodzRys,int Szerokosc)
 {
  bool rezultat = true;
  ...
  _InfoPlikow.push_back(InfoFileToDraw(NazwaPliku,RodzRys,Szerokosc));
  return rezultat;
 }
 ...
 */
                                             ...
                                             )
 {
  ...
  list<InfoFileToDraw>::const_iterator Nazwy = _InfoPlikow.begin();
...
std::string LaczeDoGNUPlota::ZapiszUstawienieRotacjiISkali() const
{
 ...
 {
   list<InfoFileToDraw>::iterator Nazwy = _InfoPlikow.end();
