 ...
 * \brief Plik zawiera definicję klasy realizującej interfejs komunikacyjny do programu gnuplot.
 */
...
namespace PzG {
   ...
   *                 
   */
  enum DrawingMode { TR_2D,  TR_3D }; //TrybRysowania
  /*!
   * \brief Sposób rysowania linii
   * 
   * Określa sposób rysowania linii.
   */
  enum DrawingType { RR_Ciagly, RR_Punktowy }; //Rodzaj Rysowania
  ...
  class InfoFileToDraw {
    public:
      ...
      */
      InfoFileToDraw(const char* NazwaPliku, DrawingType  RodzRys, int Szerokosc)
      {
      ...
      */
      DrawingType WezRodzRys() const { return _RodzRys; }
     /*!
...
class LaczeDoGNUPlota {
 protected:
  /*!
   * Lista nazw plików z danymi dla \e gnuplota.
   ...
   */
  static std::list<InfoFileToDraw>  _InfoPlikow;
 ...
 public:
   ...
   */
  void ZmienTrybRys(DrawingMode  Tryb) { _TrybRys = Tryb; }
   ...
   */
  DrawingMode WezTrybRys() const { return _TrybRys; }
  ...
  void WyswietlajKomunikatyBledow( bool Tryb = true );
    ...
    *            dla \e gnuplota.
    */
   bool DodajNazwePliku( const char * NazwaPliku, DrawingType RodzRys = RR_Ciagly, int Szerokosc = 1);
